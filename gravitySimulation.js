...
class Planet {
  ...
  movePlanet() {    
    ...
    if (this.velocity[0] + this.x < 0 || this.velocity[0] + this.x > 600)
    {
      this.velocity[0] *= -1;
      this.accel[0] *= -1;

    }
    if (this.velocity[1] + this.y < 0 || this.velocity[1] + this.y > 600)
    {
      this.velocity[1] *= -1;
      this.accel[1] *= -1;
    }
...
function calculation(mass1, mass2) {
  ...
  if (distance(mass1, mass2) < (mass1.radius / 1.2))
    f = 0;
...
function Reset() {
  planets = [];
  planets.push(new Planet(random(210, 390), random(210, 390), 10, color("blue"), 10));
  planets.push(new Planet(random(210, 390), random(210, 390), 10, color("yellow"), 10));
  planets.push(new Planet(random(210, 390), random(210, 390), 10, color("red"), 10));
  prevPoints = [ [], [], [] ];
}
...
function setup() {
  ...
  button.position(265, 1070);
  button.mousePressed(Reset);

  // create sliders
  sliderMass = createSlider(5, 20, 10);
  sliderMass.position(125, 1080);
  sliderGravity = createSlider(5, 25, 5);
  sliderGravity.position(360, 1080);

  planets.push(new Planet(random(150, 450), random(150, 450), 10, color("blue"), 10));
  planets.push(new Planet(random(150, 450), random(150, 450), 10, color("yellow"), 10));
  planets.push(new Planet(random(150, 450), random(150, 450), 10, color("red"), 10));
